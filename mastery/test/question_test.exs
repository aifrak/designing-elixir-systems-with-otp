defmodule QuestionTest do
  ...
  def eventually_match(generators, answer) do
    # We use Stream.repeatedly to let our generator build an endless stream of substitutions,
    # and we then convert the stream to an enumerable with Enum.find.
    # That means we'll get random numbers until we hit the one we're looking for.
